...
import { healthColor, healthLabel, healthFactors } from "@/lib/mockData";
    ...
    <Card>
      <CardContent className="flex flex-col gap-6 p-6 md:flex-row md:items-center">
        ...
        <div className="flex-1 space-y-3">
          {healthFactors.map((f) => {
            const bar =
              f.status === "ok" ? "bg-emerald-500" : f.status === "warn" ? "bg-amber-500" : "bg-rose-500";
            const Icon = f.status === "ok" ? CheckCircle2 : f.status === "warn" ? AlertTriangle : XCircle;
            const iconColor =
              f.status === "ok" ? "text-emerald-500" : f.status === "warn" ? "text-amber-500" : "text-rose-500";
            return (
