...
// ── Team: 4 members with Turkcell emails ─────────────────────────────────────

const teamMembers = [
  {
    name: "Dursun Koc",
    email: "dursun.koc@turkcell.com.tr",
    skills: ["React", "Node.js", "TypeScript", "PostgreSQL", "AWS", "System Design"],
    current_load: 50,
    domain_history: [{ domain: "Full Stack", sprints: 8 }, { domain: "BE", sprints: 6 }, { domain: "Architecture", sprints: 5 }],
  },
  {
    name: "Muhammet Yusuf Namdar",
    email: "muhammet.namdar@turkcell.com.tr",
    skills: ["Java", "Spring Boot", "Kafka", "Microservices", "PostgreSQL", "Elasticsearch"],
    current_load: 65,
    domain_history: [{ domain: "BE", sprints: 7 }, { domain: "DB", sprints: 4 }, { domain: "Search", sprints: 3 }],
  },
  {
    name: "Sevinc Besdas",
    email: "sevinc.besdas@turkcell.com.tr",
    skills: ["React", "TypeScript", "Playwright", "Jest", "Accessibility", "Design System"],
    current_load: 40,
    domain_history: [{ domain: "FE", sprints: 6 }, { domain: "QA", sprints: 5 }, { domain: "UX", sprints: 3 }],
  },
  {
    name: "Izzet Ozturk",
    email: "izzet.ozturk@turkcell.com.tr",
    skills: ["Kubernetes", "Docker", "Terraform", "GitHub Actions", "Prometheus", "AWS"],
    current_load: 30,
    domain_history: [{ domain: "DevOps", sprints: 7 }, { domain: "Infra", sprints: 5 }, { domain: "SRE", sprints: 4 }],
  },
];
...
// ── Subtasks (assignees match the 4-person team) ─────────────────────────────
...
async function insertTeam() {
  // Try with email column; if it errors (column missing), retry without email
  ...
  if (r.error && /email/i.test(r.error.message)) {
    ...
    console.warn("   Run this in Supabase SQL Editor to add it:");
    console.warn("   alter table team_members add column if not exists email text;");
