...
import { backlog, type BacklogTask } from "../../lib/mockData";

export function Planning() {
  const [selectedId, setSelectedId] = useState<string>(backlog[0].id);
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_360px]">
      ...
      <Card className="h-fit sticky top-4">
        ...
        <CardContent className="space-y-4">
          ...
          {selected.passes ? (
            <Button className="w-full" disabled={!selected.passes}>
              <Plus className="h-4 w-4" /> Add to Sprint
            </Button>
          ) : (
            <div className="rounded-md border border-rose-500/40 bg-rose-500/10 p-3 text-rose-400">
              <div className="flex items-center gap-2">
                <XCircle className="h-4 w-4" /> Anti-Bullshit gate rejected
              </div>
  ...
  return (
    <button
      ...
      className={`w-full text-left rounded-lg border p-3 transition-colors ${
        active ? "border-indigo-500 bg-indigo-500/5" : "border-border hover:bg-accent/50"
      }`}
    >
        ...
        <div className="flex items-start gap-3">
          <Icon
            className={`mt-0.5 h-4 w-4 ${
              task.type === "bug" ? "text-rose-500" : "text-sky-500"
            }`}
          />
          <div>
            <div>{task.title}</div>
      ...
      {!task.passes && (
        <div className="mt-2 text-rose-400">⛔ {task.rejectReason}</div>
      )}
    </button>
  );
}
